...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("EXACT_ONLINE_ACCOUNTANCY_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("EXACT_ONLINE_ACCOUNTANCY_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "exact-online-accountancy-pp-cli/feedback")
...
When ` + "`EXACT_ONLINE_ACCOUNTANCY_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`EXACT_ONLINE_ACCOUNTANCY_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
maintainer sees it.`,
		Example: `  exact-online-accountancy-pp-cli feedback "the --since flag is inclusive but docs say exclusive"
  exact-online-accountancy-pp-cli feedback --stdin < notes.txt
  exact-online-accountancy-pp-cli feedback list --limit 10`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			entry := FeedbackEntry{
				Text:      text,
				CLI:       "exact-online-accountancy-pp-cli",
				Version:   version,
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  exact-online-accountancy-pp-cli feedback list
  exact-online-accountancy-pp-cli feedback list --limit 5
  exact-online-accountancy-pp-cli feedback list --json`,
