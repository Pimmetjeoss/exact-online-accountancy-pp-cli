...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  exact-online-accountancy-pp-cli doctor
  exact-online-accountancy-pp-cli doctor --json
  exact-online-accountancy-pp-cli doctor --fail-on warn
  exact-online-accountancy-pp-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if credentialRefused {
				...
				if cfg != nil {
					...
					if !configured {
						...
						report["auth_hint"] = "exact-online-accountancy-pp-cli login"
					} else {
						authConfigured = true
			...
			if os.Getenv("EXACT_ONLINE_ACCOUNTANCY_OAUTH2") != "" {
				authEnvSet = append(authEnvSet, "EXACT_ONLINE_ACCOUNTANCY_OAUTH2")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "EXACT_ONLINE_ACCOUNTANCY_OAUTH2")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "exact-online-accountancy-pp-cli", suggestion)
						} else {
