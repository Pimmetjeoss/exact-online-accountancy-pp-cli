...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:         "auth",
		Short:       "Manage authentication for Exact Online Accountancy",
...
func newAuthSetupCmd(_ *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "  exact-online-accountancy-pp-cli auth setup\n  exact-online-accountancy-pp-cli auth setup --launch",
		RunE: func(cmd *cobra.Command, args []string) error {
			w := cmd.OutOrStdout()
			fmt.Fprintln(w, "No setup URL is configured for this CLI; check the API's docs.")
			fmt.Fprintln(w, "")
			fmt.Fprintln(w, "Then run:")
			fmt.Fprintln(w, "  exact-online-accountancy-pp-cli login")
			if !launch {
				return nil
			}
...
func newAuthLoginCmd(flags *rootFlags) *cobra.Command {
	...
	cmd.Flags().StringVar(&clientID, "client-id", os.Getenv("EXACT_ONLINE_ACCOUNTANCY_CLIENT_ID"), "OAuth2 client ID")
	cmd.Flags().StringVar(&clientSecret, "client-secret", os.Getenv("EXACT_ONLINE_ACCOUNTANCY_CLIENT_SECRET"), "OAuth2 client secret")
...
func resolveOAuthCredentials(cmd *cobra.Command, flags *rootFlags, cfg *config.Config, clientID, clientSecret string) (string, string, error) {
	...
	if flags.noInput {
		return "", "", fmt.Errorf("OAuth2 client ID is required; pass --client-id, set EXACT_ONLINE_ACCOUNTANCY_CLIENT_ID, or run 'exact-online-accountancy-pp-cli login' without --no-input")
	}
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !authed {
				fmt.Fprintf(w, "  %s Not authenticated. Run 'exact-online-accountancy-pp-cli login' to authenticate.\n", red("FAIL"))
				return nil
			}
			...
			if cfg.TokenExpiry.IsZero() {
				...
				if cfg.RefreshToken != "" {
					...
					fmt.Fprintf(w, "  %s Token expired. Run 'exact-online-accountancy-pp-cli login' to re-authenticate.\n", red("FAIL"))
				}
			}
