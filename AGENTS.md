# Exact Online Accountancy Printed CLI Agent Guide

This directory is a generated `exact-online-accountancy-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
exact-online-accountancy-pp-cli doctor --json
exact-online-accountancy-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
exact-online-accountancy-pp-cli which "<capability>" --json
exact-online-accountancy-pp-cli <command> --help
```

Add `--agent` to command invocations for JSON, compact output, non-interactive defaults, and no color:

```bash
exact-online-accountancy-pp-cli <command> --agent
```
...
```bash
exact-online-accountancy-pp-cli <command> --help
exact-online-accountancy-pp-cli <command> --dry-run --agent
```
...
3. After answering, always fire `exact-online-accountancy-pp-cli teach --query "<question>" --resource <id> --resource-type <type> &` in the background - teaching is unconditional and is the anchor that triggers playbook synthesis. Teach the structural question with identifiers stripped (no names, emails, phone numbers, account ids); the CLI warns on obvious PII shapes but does not block.
...
Disable the loop with `--no-learn` per-invocation or `EXACT_ONLINE_ACCOUNTANCY_NO_LEARN=true` for the whole session - useful for deterministic agent flows that don't want a learning row to silently change subsequent query results.
